...
void World::AddPrimitive(PrimitiveComponent* newPrimitive)
{
...
void World::processInput() {
	...
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
	{
		...
		if (!defaultTexture) defaultTexture = new Texture2D("Sources/Ressources/container.jpg");
		if (!defaultMaterial) defaultMaterial = new Material("Sources/Shaders/defaultVertexShader.vs", "Sources/Shaders/defaultFragmentShaderss.fs", { defaultTexture });
		if (!cubeMesh) cubeMesh = new StaticMesh("Sources/Ressources/cube.obj", { defaultMaterial });
...
void World::UpdateWorld(double deltaSecond)
{
	...
	{
		primitives[primIndex]->Draw(worldCamera->GetViewMatrix(), projection);

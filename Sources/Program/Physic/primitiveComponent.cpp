...
void PrimitiveComponent::Tick()
{
	SceneComponent::Tick();
	//TOFIX
// 	if (GetScene()->bDrawBounds)
// 	{
// 		ProfileStat("PushVertex");
// 		GetScene()->GetPhysicWorld()->getDebugDrawer()->drawSphere(GetWorldBounds().GetOrigin().ToBulletVector(), GetWorldBounds().GetBoundRadius(), SVector3(.5f, 1, 0).ToBulletVector());
// 		GetScene()->GetPhysicWorld()->getDebugDrawer()->drawBox(GetWorldBounds().GetBoxMin().ToBulletVector(), GetWorldBounds().GetBoxMax().ToBulletVector(), SVector3(0, .4f, 1).ToBulletVector());
// 	}
}
...
void PrimitiveComponent::DrawEditorWindow()
{
	...
	//GetScene()->GetDebugDrawer()->drawBox(GetWorldBounds().GetBoxMin().ToBulletVector(), GetWorldBounds().GetBoxMax().ToBulletVector(), btVector3(1.f, 0.6f, 0.f));
}

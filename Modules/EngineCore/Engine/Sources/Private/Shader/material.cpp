...
void Material::UpdateMaterialDefaults(Scene* DrawScene)
{
	...
	shader_data.directionalLightCount = (unsigned int)directionalLights.size();
	shader_data.pointLightCount = (unsigned int)pointLights.size();
	shader_data.spotLightCount = (unsigned int)spotLights.size();

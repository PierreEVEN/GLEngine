...
{
	if ((unsigned int)inputFileStream->tellg() + sizeof(unsigned int) >= (unsigned int)fileSize) return false;
	unsigned int nameBufferSize;
	inputFileStream->read(reinterpret_cast<char*>(&nameBufferSize), sizeof(unsigned int));

	if ((unsigned int)inputFileStream->tellg() + nameBufferSize >= (unsigned int)fileSize) return false;
...
{
	GLAssetIO::AppendField<char*>(newFileStream, "AssetType", (char*)(assetType.data()), (unsigned int)assetType.size() + 1);
	GLAssetIO::AppendField<char*>(newFileStream, "AssetName", (char*)(newAssetName.data()), (unsigned int)newAssetName.size() + 1);
}

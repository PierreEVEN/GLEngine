...
std::string AssetLibrary::GetExtension(const std::string& filePath)
{
	...
	for (int i = (unsigned int)filePath.size() - 1; i >= 0 && !bFoundDot; --i)
	{
		if (filePath.at(i) == '.')
		{
			bFoundDot = true;
...
{
	int extensionLocation = (unsigned int)extension.size() - 1;
	for (int i = (unsigned int)filePath.size() - 1; i >= 0; --i)
	{
...
std::string AssetLibrary::RemoveExtension(const std::string& filePath)
{
	...
	int i;
	for (i = (unsigned int)filePath.size() - 1; i >= 0 && filePath[i] != '.'; --i) { }
	for (int j = 0; j < i; ++j)
	{
